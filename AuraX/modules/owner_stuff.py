...
from AuraX import AuraXRobot_VERSION, bot, dp
from AuraX.decorator import COMMANDS_ALIASES, REGISTRED_COMMANDS, register
from AuraX.modules import LOADED_MODULES
from AuraX.services.mongo import db, mongodb
...
async def restart_bot(message):
    await message.reply("AuraXRobot will be restarted...")
    args = [sys.executable, "-m", "AuraX"]
...
async def stats(message):
    if message.from_user.id in devs:
        text = f"<b>AuraXRobot {AuraXRobot_VERSION} stats</b>\n"
