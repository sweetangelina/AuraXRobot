...
from AuraX.modules.utils.disable import disableable_dec
...
async def get_start_func(message, strings, edit=False):
    ...
    buttons.add(
        InlineKeyboardButton(strings["btn_channel"], url="https://t.me/AuraXUpdates"),
        InlineKeyboardButton(
            "👥 Support Grup", url="https://t.me/AuraXRobotSupport_Official"
        ),
    )
    buttons.add(
        InlineKeyboardButton(
            "👸🏼 Add AuraX to your group",
            url=f"https://telegram.me/AuraXRobot?startgroup=true",
        )
    )
...
async def help_cmd_g(message, strings):
    ...
    button = InlineKeyboardMarkup().add(
        InlineKeyboardButton(text=text, url="https://t.me/AuraXRobot?start")
    )
    await message.reply(strings["help_header"], reply_markup=button)
