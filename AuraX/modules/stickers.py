...
from AuraX.services.events import register as AuraXRobot
...
@AuraXRobot(pattern="^/searchsticker (.*)")
async def _(event):
...
async def _(event):
    ...
    packshortname = f"AuraX_stickers_{userid}"
    kanga = await event.reply("Hello, This Sticker Looks Noice. Mind if AuraXRobot steal it")
    ...
    if is_a_s:
        ...
        packshortname = f"AuraX_animated_{userid}"
